...
public enum SlashCommandMenu {
    ...
    public static func modelSuggestions(for kind: AgentConfig.Kind, query: String, catalog: [String] = []) -> [String] {
        // 有动态目录就用它（前置 “default” 让用户可选默认），否则回落内置预设。
        let base = catalog.isEmpty ? modelPresets(for: kind) : (["default"] + catalog)
    ...
    /// 人类可读的模型名：去供应商（取最后一个 “/” 之后）、把 “-” 换空格、各段首字母大写；
    /// 命中缩写词表的段用规范大小写。例：“opencode-go/deepseek-v4-flash” → “Deepseek V4 Flash”；
    /// “openai/gpt-4o” → “GPT 4o”；“default” → “默认模型”。
    public static func modelDisplayName(_ model: String) -> String {
        ...
        let parts = name.split(separator: "-").map { raw -> String in
            let part = String(raw)
            if let acronym = modelAcronyms[part.lowercased()] { return acronym }
